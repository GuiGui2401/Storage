...
def get_train_test(train_ratio, pathData):
    ...
    #Initialisation du premier tableau pour avoir les bonnes dimensions pour la suite
    ...
    #Ajoute du reste des fichiers numpy de nos classes
...
def main():
    ...
    pathData = os.path.join(module_dir, 'numpy')
    trainRatio = 0.8
    epochs = 1000
    batch_size = 16
    earlyStopPatience = 5
    ...
    #P retourner 4 metrics de suivi a chaque iteration
    csv_logger = CSVLogger(os.path.join(module_dir, 'logs\\log_moModel.csv'), append=True, separator=',')
    ...
    #sauvegarde du model a chaque iteration si il est meilleur que le precedent
    check = ModelCheckpoint(os.path.join(module_dir, 'trainedModel\\moModel.hdf5'), monitor='val_loss', verbose=0,
    ...
    #Verification des dimensions de nos données
    ...
    model.compile(loss=keras.losses.categorical_crossentropy,
                  optimizer=tf.keras.optimizers.Adamax(lr=0.001, beta_1=0.9, beta_2=0.999, weight_decay=None),
                  metrics=['accuracy'])
