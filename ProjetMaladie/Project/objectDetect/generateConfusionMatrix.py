...
def generateMatrix(model, datasetTestPath, imageSize, destinationMatrix):
	...
	cmap = plt.cm.Blues
	classes = ['daisy', 'dandelion', 'rose', 'tournesol', 'tulip']
...
def main():
	...
	modelPath = os.path.join(current_dir, 'trainedModel\\moModel.hdf5')
	datasetTestPath = os.path.join(current_dir, 'datasetTest')
	destinationMatrix = os.path.join(current_dir, 'graph')
